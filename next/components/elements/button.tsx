...
export const Button: React.FC<ButtonProps> = ({
  ...
  const variantClass =
    variant === "simple"
      ...
      : variant === "primary"
      ? "bg-secondary relative h-14 z-10 font-bold uppercase border-4 border-mainText text-primary text-sm md:text-sm transition duration-200 px-4 py-2  flex items-center justify-center shadow-[0px_-1px_0px_0px_#FFFFFF60_inset,_0px_1px_0px_0px_#FFFFFF60_inset  hover:-translate-y-1 active:-translate-y-0"
      : variant === "muted"
  ...
  return (
    <Tag
      className={cn(
        "bg-secondary relative z-10 bg-transparent border border-mainText text-white text-sm md:text-sm transition font-medium duration-200  px-4 py-2  flex items-center justify-center ",
        variantClass,
        className
      )}
      {...props}
